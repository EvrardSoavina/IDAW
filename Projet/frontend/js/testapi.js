$(document).ready(function() {
    $('#post-form').submit(function(event) {
        ...
        let login = login // Nom d'utilisateur de la session PHP
        ...
        $.ajax({
            type: 'GET',
            url: apifolder + '/backend/type_repas.php?nom_repas=' + nom_repas,
            success: function(data) {
                responseObject = JSON.parse(data);
                id_type_repasss = responseObject[0].id_type_repas;
                $.ajax({
                    type: 'GET',
                    url: apifolder + '/backend/aliments.php?nom=' + nom_aliment,
                    success: function(data) {
                        responseObject = JSON.parse(data);
